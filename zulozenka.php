 ...
 *
 * @version $Id: weight_countries.php 10852 2023-05-24 10:59:41Z Milbo $
 * @package VirtueMart
...
class PlgvmshipmentZulozenka extends vmPSPlugin {
	...
	function plgVmConfirmedOrder (VirtueMartCart $cart, $order) {
	    ...
	    $branch_id = JRequest::getVar('ulozenka_pobocky'); 
		...
		$values['branch_name'] = $pobocka_name;
		
		$values['shipment_name'] = $this->renderPluginName ($method).' - '.$pobocka_name;
...
$html = array();
$htmlq = array();
	//$methodSalesPrice     = $this->calculateSalesPrice($cart, $method);   
	...
	private function getBranch($branch_id) {
		...
		if ($branch_id <> 'Vyberte pobočku'){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select($db->quoteName(['name']));
			$query->from($db->quoteName('#__ulozenka_branches'));
			$query->where($db->quoteName('branch_id')." = ". $branch_id);
			$db->setQuery($query);
			$result = $db->loadResult();
			$pobocka_moje = str_replace('"','',$result);
			$pobocka_name = $pobocka_moje;
			return $pobocka_name;
		}
		else {
			return FALSE;
